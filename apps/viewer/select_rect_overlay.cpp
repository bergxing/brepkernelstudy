...
void SelectRectOverlay::paintEvent(QPaintEvent* event) {
  Q_UNUSED(event);
  if (select_rect_.width() < 1 || select_rect_.height() < 1) return;
  ...
  p.drawRect(select_rect_.adjusted(0, 0, -1, -1));
}
