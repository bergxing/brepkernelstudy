...
TEST(SnapFeedback, InitialCreateToolHoverRunsSnapResolution)
{
  ...
  CreateBoxTool box;
  box.OnStart(ctx);
  box.OnMouseMove(ctx, 400.0f, 300.0f);

  CreateSphereTool sphere;
  sphere.OnStart(ctx);
  sphere.OnMouseMove(ctx, 400.0f, 300.0f);
...
{
  SnapSession session;
  session.ActiveSnap = SnapKind::Endpoint;
  CommandContext ctx;
  ...
  EXPECT_FALSE(session.ActiveSnap.has_value());
...
TEST(AccuSnapGrid, QuantizesWorkplaneCoordinatesToSpacing)
{
  const auto candidate =
      MakeGridCandidate(Point3d{1.24, 0.0, -2.76}, 0.5);
...
{
  EXPECT_FALSE(MakeGridCandidate(Point3d{}, 0.0).has_value());
  EXPECT_FALSE(MakeGridCandidate(Point3d{}, -1.0).has_value());
  EXPECT_FALSE(
      MakeGridCandidate(Point3d{}, std::numeric_limits<double>::infinity())
          .has_value());
}
...
TEST(Picking, IntersectEdgesHitsWireNearCursor)
{
  Camera camera;
  camera.target = Point3d{0.0, 0.0, 0.0};
  camera.distance = 6.0f;
  camera.yaw_deg = 0.0f;
  camera.pitch_deg = -90.0f;

  EdgeMesh edges;
  edges.Positions = {Point3d{-1.0, 0.0, 0.0}, Point3d{1.0, 0.0, 0.0}};

  constexpr int kW = 800;
  constexpr int kH = 600;
  float sx = 0.0f;
  float sy = 0.0f;
  ASSERT_TRUE(WorldToScreen(camera, kW, kH, Point3d{0.0, 0.0, 0.0}, sx, sy));

  Point3d origin;
  Vector3d dir;
  ASSERT_TRUE(ScreenToRay(camera, kW, kH, sx, sy, origin, dir));

  double t = 0.0;
  EXPECT_TRUE(IntersectEdges(camera, kW, kH, sx, sy, 12, origin, dir, edges,
                             Point3d{}, t));
  EXPECT_GT(t, 0.0);

  EXPECT_FALSE(IntersectEdges(camera, kW, kH, sx, sy + 80.0f, 12, origin, dir,
                              edges, Point3d{}, t));
}
