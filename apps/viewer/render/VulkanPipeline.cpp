...
void VulkanRenderer::create_descriptors()
{
  ...
  m_dev->vkCreateDescriptorSetLayout(device, &lci, nullptr, &m_descLayout);
  ...
  m_dev->vkCreateDescriptorPool(device, &pci, nullptr, &m_descPool);
  ...
  m_dev->vkAllocateDescriptorSets(device, &ai, sets);
  m_descSet = sets[0];
  m_selectionDescSet = sets[1];
  m_axisDescSet = sets[2];

  VkDescriptorImageInfo ii{};
  ii.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
  ii.imageView = m_albedo.view;
  ii.sampler = m_albedo.sampler;

  VkDescriptorImageInfo sel_ii{};
  sel_ii.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
  sel_ii.imageView = m_selectionAlbedo.view;
  sel_ii.sampler = m_selectionAlbedo.sampler;

  VkDescriptorBufferInfo scene_bi{};
  scene_bi.buffer = m_ubo.buffer;
  scene_bi.offset = 0;
  scene_bi.range = sizeof(Ubo);

  VkDescriptorBufferInfo sel_bi{};
  sel_bi.buffer = m_selectionUbo.buffer;
  sel_bi.offset = 0;
  sel_bi.range = sizeof(Ubo);

  VkDescriptorBufferInfo axis_bi{};
  axis_bi.buffer = m_axisUbo.buffer;
  axis_bi.offset = 0;
  ...
  writes[2].dstSet = m_selectionDescSet;
  ...
  writes[3].dstSet = m_selectionDescSet;
  ...
  writes[5].dstSet = m_axisDescSet;
  writes[5].dstBinding = 1;
  writes[5].descriptorCount = 1;
  writes[5].descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
  writes[5].pImageInfo = &ii;

  m_dev->vkUpdateDescriptorSets(device, 6, writes, 0, nullptr);
}
...
void VulkanRenderer::create_pipelines()
{
  ...
  m_dev->vkCreatePipelineLayout(device, &plci, nullptr, &m_pipelineLayout);

  enum class PipeKind
  {
      Mesh, Line, Axis, PreviewFill 
  };
                           ...
                           VkPipeline* out)
  {
    ...
    gp.renderPass = m_window->defaultRenderPass();

    if (m_dev->vkCreateGraphicsPipelines(device, m_pipelineCache, 1, &gp, nullptr,
                                        out) != VK_SUCCESS)
    {
  ...
  make_pipeline(vert, frag, PipeKind::Mesh, &m_triPipeline);
  make_pipeline(line_vert, line_frag, PipeKind::Line, &m_linePipeline);
  make_pipeline(axis_vert, axis_frag, PipeKind::Axis, &m_axisPipeline);
  ...
  m_dev->vkDestroyShaderModule(device, line_vert, nullptr);
  m_dev->vkDestroyShaderModule(device, line_frag, nullptr);
  m_dev->vkDestroyShaderModule(device, axis_vert, nullptr);
  m_dev->vkDestroyShaderModule(device, axis_frag, nullptr);
}
