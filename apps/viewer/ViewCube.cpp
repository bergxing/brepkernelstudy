...
ViewCubeWidget::ViewCubeWidget(QWidget* parent) : QWidget(parent)
{
  ...
  connect(timer, &QTimer::timeout, this, [this] {
    if (!m_camera) return;
    if (m_camera->yaw_deg != m_lastYaw || m_camera->pitch_deg != m_lastPitch ||
        m_camera->framed != m_lastFramed ||
        m_camera->ortho != m_lastOrtho)
        {
      m_lastYaw = m_camera->yaw_deg;
      m_lastPitch = m_camera->pitch_deg;
      m_lastFramed = m_camera->framed;
      m_lastOrtho = m_camera->ortho;
      update();
    }
  });
  timer->start(33);
...
void ViewCubeWidget::paintEvent(QPaintEvent*)
{
  ...
  std::sort(projected.begin(), projected.end(),
            [](const ProjFace& a, const ProjFace& b)
  {
  ...
  for (const ProjFace& pf : projected)
  {
    ...
    if (pf.id == m_hover) fill = fill.lighter(125);
    p.setBrush(fill);
  ...
  const bool home_hover = m_hover == FaceId::Home;
...
{
  if (!m_camera || event->button() != Qt::LeftButton) return;
  ...
  if (code != 0)
  {
    m_camera->set_standard_view(code);
    notify_redraw();
    update();
  }
}
