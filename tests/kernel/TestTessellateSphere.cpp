...
TEST(TessellateSphere, DefaultMeshScaleAndNormals)
{
  ...
  // CDT + interior UV lattice (not a rigid (nu+1)Ã—(nv+1) grid).
...
{
  Model model;
  Body* body = MakeBox(model, BoxSpec{.Min = {0, 0, 0}, .Max = {1, 1, 1}});
  ASSERT_NE(body, nullptr);
  const TriangleMesh mesh = tessellate_body(*body);
  EXPECT_EQ(mesh.indices.size() / 3, 12u);  // 6 faces Ã— 2 tris
