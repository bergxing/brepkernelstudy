...
Body* build_from_quads(Model& model, const std::vector<FaceQuad>& quads,
  ...
  auto get_edge = [&](Vertex* va, Vertex* vb) -> std::pair<Edge*, bool> {
    ...
    Edge* e =
        model.make_edge(curve, va, vb, 0.0, curve->length(), tol);
    edges.emplace(key, e);
    return {e, true};
  };
...
BooleanResult evaluate_box_boolean(BooleanOp op, Model& model, const Body& a,
                                   const Body& b, const BooleanContext& ctx) {
  BooleanResult result;
  result.mode = BooleanEvalMode::AnalyticPair;

  const auto sa = recognize_axis_aligned_box(a, ctx);
  const auto sb = recognize_axis_aligned_box(b, ctx);
  if (!sa || !sb) {
    result.diagnostics =
  ...
  // Fast path: single AABB result → reuse make_box.
