...
Body* build_from_quads(Model& model, const std::vector<FaceQuad>& quads,
  ...
  auto get_vert = [&](const Point3d& p) -> Vertex* {
    ...
    Point* pt = model.make_point(p);
    Vertex* v = model.make_vertex(pt, tol);
    verts.emplace(key, v);
    return v;
  };
  ...
  auto get_edge = [&](Vertex* va, Vertex* vb) -> std::pair<Edge*, bool> {
    ...
    LineCurve* curve = model.make_line(va->position(), vb->position());
    Edge* e = model.make_edge(curve, va, vb, 0.0, curve->length(), tol);
    edges.emplace(key, e);
    return {e, true};
  };
  ...
  for (const FaceQuad& q : quads) {
    ...
    PlaneSurface* surf =
        model.make_plane(q.corners[0], u, v, name + "_f" + std::to_string(face_i));
    Face* face = model.make_face(surf, Orientation::Forward,
                                 name + "_face" + std::to_string(face_i));
    shell->faces.push_back(face);
    Loop* loop =
        model.make_loop(face, LoopType::Outer, name + "_loop" + std::to_string(face_i));
    ...
    for (int k = 0; k < 4; ++k) {
      ...
      const Point2d uva{surf->param_of(a->position())};
      const Point2d uvb{surf->param_of(b->position())};
      Curve2d* pc = model.make_line2d(uva, uvb);
      ces[k] = model.make_coedge(edge, sense, pc);
    }
  ...
  for (auto& [key, edge] : edges) {
    ...
    Model::pair_partners(edge->radial[0], edge->radial[1]);
  }

  if (!validate_body(*body).ok()) return nullptr;
  return body;
}
...
void collect_profile_coords(const PlanarPrismSpec& prism,
  ...
  auto add_uv = [&](const Point2d& uv, double h) {
    const Point3d p =
        prism.plane.origin + prism.plane.u_axis * uv.u() +
        prism.plane.v_axis * uv.v() + prism.plane.normal * h;
...
BooleanResult evaluate_prism_box_boolean(BooleanOp op, Model& model,
  ...
  auto in_prism = [&](const Point3d& mid) {
    return classify_point_in_prism(prism, mid, eps) != SolidClass::Out;
  };
  auto in_box = [&](const Point3d& mid) {
    return classify_point_in_box(box, mid, eps) != SolidClass::Out;
  };
  ...
  result.body = build_from_quads(model, quads, out_name,
