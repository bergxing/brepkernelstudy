...
SolidClass classify_point_in_box(const BoxSpec& box, const Point3d& p,
                                 double eps) {
...
SolidClass classify_point_in_prism(const PlanarPrismSpec& prism, const Point3d& p,
                                   double eps) {
  ...
  const Point2d uv{d.dot(prism.plane.u_axis), d.dot(prism.plane.v_axis)};
