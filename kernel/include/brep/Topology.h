...
// ---------------------------------------------------------------------------
// Topology skeleton: cross-links are non-owning raw pointers; unique ownership
// lives in Model (see cpp-core-guidelines/project-overrides.md).
// ---------------------------------------------------------------------------
...
public:
    ...
    [[nodiscard]] double ParamAt(Orientation sense, double localT) const noexcept
    {
        // localT in [0,1] along the coedge direction
        return sense == Orientation::Forward ? (T0 + (T1 - T0) * localT)
                                             : (T1 + (T0 - T1) * localT);
    }
};
...
public:
    ...
    /// Only for BodyType::Wire. Solid/Sheet must keep this empty.
    std::vector<Edge*> WireEdges;
