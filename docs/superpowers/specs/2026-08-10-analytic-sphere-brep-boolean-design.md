...
**退出标准：** 弯曲布尔可用于学习演示；仍非完整商用 CAD 内核。 — **已满足 MVP（T3.1～T3.6：`Box∩Sphere` ⅛-ball）**

### Phase 4 —— 扩展曲面（可选）

- `CylinderSurface` 与更多求交对 — **T4.1/T4.2 已完成（偏轴球–柱曲线待扩展）**
- 更好的 pcurve、命名（`TopologyRef`）— **T4.3 约定已文档化**
- 性能（BVH）— **T4.4 完成**（Median + binning SAH + 布尔宽相诊断挂钩）
...
- [x] MVP：八分之一球交直接建修剪球面 Face（3 圆弧 Outer）；通用缝边印记后续
- [x] **测试：** 印记/建面后球面拓扑可校验（`OctantIntersectValidates`）

#### T3.4 解析分类（球/盒）

- [x] `classify_point_in_sphere` + `recognize_analytic_sphere`
- [x] **测试：** 球内/外/上；盒面相对球抽样可后续扩展

#### T3.5 端到端弯曲布尔

- [x] 打通：`Box∩Sphere`（球心在 `box.min`、盒包含 +++ 八分球）
- [ ] Viewer 可选演示路径
- [x] **验收：** 结果 `validate_body` 通过；失败有明确诊断

#### T3.6 Phase 3 验收门禁

- [x] 弯曲单元测试 + 至少 1 个 Body 级弯曲用例通过
- [x] 文档勾选 Phase 3 退出标准

#### 为何 `Sphere∪Sphere` 比 `Box∩Sphere` 难（后续扩展笔记）

当前 MVP 只打通 **`Box∩Sphere` → ⅛ 球**（球心在 `box.min`、盒包住 +++ 八分球）：结果是 **3 个平面扇区 + 1 个球面三角片**，可按已知构型**直接建壳**，不必在完整单缝球上做通用印记。

`Sphere∪Sphere`（及一般 `Sphere−Sphere`）难点不在求交公式（T3.2 已给出圆 / 切点 / 分离 / 包含 / 重合），而在 **拓扑重建**：

1. **双缝参数域冲突**  
   每个解析球是「双极点 + 一条经向缝 + 单 Face」。并集后外壳上通常保留 **两块球面片**，各自仍有（或需新建）缝边；交线圆相对两套 `(u,v)` 的参数位置不同，不能共用一套「八分之一特解」。

2. **交线圆必须印到两边球面**  
   交圆要同时成为两球 Face 的边界边：在各自 UV 上写 **pcurve**（一般不是直线），并在 3D 上共享同一条 `CircleCurve` Edge。圆若穿过某球的缝或极点，还要 **打断 seam / 处理极点退化**，否则 Outer/Inner 环无法闭合。

3. **选面与壳连通**  
   Union 保留「在 A 外或在 B 外」的球面片：每球被交圆切成帽/带，要丢掉被对方球盖住的部分，再把两壳沿交圆 **缝成单一封闭壳**。透镜区、包含、相切、近相切（fuzzy）都会改变面片数量与环类型（常出现 Inner）。

4. **不能走盒式 arrangement**  
   Phase 2 的盒/棱柱布尔靠正交切平面网格吐 **平面四边形**。球–球结果面是弯曲的，网格 CSG 无法直接得到合法 `SphereSurface` Face；必须走 **求交 → 印记/分割 → 分类 → 重建**（规格 B3），或为每种构型写特解（不可扩展）。

5. **显示与校验更脆**  
   修剪后的球面片三角化要尊重弧边与缝；`validate_body` 对径向度数、环闭合、伙伴边更敏感。任一缝/圆参数跳变都会表现为「软失败有诊断」或坏壳。

**相对难度（同仓库现状）：**

| 用例 | 求交 | 拓扑 | 现状 |
|------|------|------|------|
| `Box∩Sphere`（⅛ 球） | 平面–球（可特解绕过） | 直接建 4 Face | **已通** |
| `Sphere−Box` | 多平面–球圆 | 大球面剩余 + 缝上多圆印记 | 未做 |
| `Sphere∪Sphere` | 球–球圆（已有） | 双球面印记 + 选片缝合 | **最难** |

后续若做 `Sphere∪Sphere`，建议顺序：通用「圆印到球面」（打断 seam）→ 单球被平面/圆切割的环更新 → 再双球选面缝合；勿再扩八分之一特解。
...
- [x] **T4.4.c**（可选）布尔宽相挂钩一次冒烟；注释说明 Median vs SAH
